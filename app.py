# app.py — Lyra Engine / Floria Chat (Streamlit Edition, GPT-4o + Hermes fallback)
...
# ================== ページ設定 ==================
st.set_page_config(page_title="Lyra Engine / Floria Chat", layout="wide")
st.markdown("""
<style>
...
# ================== パラメータUI ==================
st.title("❄️ Lyra Engine / Floria Chat")
...
st.markdown(f"<style>.chat-bubble {{ max-width: min(90vw, {wrap_width}ch); }}</style>",
            unsafe_allow_html=True)
...
for m in dialog:
    ...
    if role == "user":
        ...
        st.markdown(f"<div class='chat-bubble assistant'><b>{PARTNER_NAME}：</b><br>{txt}</div>", unsafe_allow_html=True)
...
do_load = col_r.button("読み込む", use_container_width=True,
